...
import React, { useEffect, useState } from 'react'; // Đảm bảo đã import useEffect
import {
    ActivityIndicator, // Để hiển thị loading khi tải tên nhà hàng
    FlatList,
    Image,
...
export default function RateProductsScreen({ navigation, route }) {
    ...
    const fetchRestaurantName = async () => {
        if (!restaurantId) {
            setRestaurantName('Không rõ nhà hàng'); // Đặt tên mặc định nếu không có ID
        ...
        try {
            ...
            if (!res.ok) {
                const errorData = await res.json(); // Cố gắng đọc lỗi từ JSON
                throw new Error(errorData.message || `Lỗi khi tải thông tin nhà hàng! Trạng thái: ${res.status}`);
            }

            const data = await res.json(); // Phân tích cú pháp phản hồi JSON
            ...
            setRestaurantName('Không thể tải tên'); // Đặt tên lỗi nếu có vấn đề
        } finally {
            setRestaurantLoading(false); // Dù thành công hay thất bại, kết thúc trạng thái loading
        }
    };
    ...
    useEffect(() => {
        fetchRestaurantName();
    }, [restaurantId]); // dependency array với restaurantId để đảm bảo nó chạy lại nếu ID thay đổi
    ...
    const handleSubmitReviews = async () => {
        ...
        setSubmitting(true); // Bắt đầu trạng thái gửi
        try {
    ...
    const renderProductReviewItem = ({ item }) => (
        <View style={styles.productReviewCard}>
            <Image
                source={{ uri: `${linkanh}${item.productImage}` }} // Hiển thị ảnh sản phẩm
            ...
            >
                ...
                <ScrollView contentContainerStyle={styles.scrollViewContent}>
                    ...
                    <FlatList
                        ...
                        scrollEnabled={false} // Không cuộn riêng FlatList, để ScrollView chính xử lý
                ...
                <TouchableOpacity
                    ...
                    disabled={submitting} // Vô hiệu hóa nút khi đang gửi
                >
                    {submitting ? (
                        <ActivityIndicator color="#fff" /> // Hiển thị loading trong nút khi đang gửi
                    ) : (
