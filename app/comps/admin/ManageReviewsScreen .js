...
const ManageReviewsScreen = ({ navigation }) => {
    ...
    const fetchAllProductReviews = useCallback(async () => {
        ...
        setError(null);
        try {
            // Gọi đến endpoint backend mới để lấy tất cả đánh giá sản phẩm đã được populate
            const response = await fetch(`${linkapi}reviews/product`);
